...
| 5.8 | **Source-video overlay with keypoint trails.** Render the analyzed video with detected pose drawn on top, scrubbable, and the metric's measured frame range highlighted. | XL | High | Needs the per-frame keypoint array to be persisted (currently only the summary survives in `athlete_lab_results.result_data`). May need Phase 2 world_landmarks for the 3D version. |
| 5.9 | **Annotate "true break frame" tool.** Admin scrubs to the actual plant frame and clicks; that timestamp is stored as ground truth on the upload. Future runs can be evaluated against it for measurement-vs-truth deviation. | L | Med (high once Phase 2 lands) | Depends on 5.8 |
