...
4. **Run the verification check in-slice. A slice may not defer its own verification into a future slice.** If the check cannot run within the slice's scope, the slice **halts** and the missing precondition becomes its own slice. "By inspection" is a defensible method when the claim is structural and runtime corroboration cannot exist yet (e.g., an additive field with no consumer cannot perturb existing serialization); when used, name the claim and its bound in the outcome doc — do not phrase it as a deferral. Slices that ship code without running their bar are marked `status: code-complete-verification-pending`, not `status: shipped`, until the verification clears. This is [F-OPS-6](../risk-register/F-OPS-6-verification-deferral-across-slice-boundaries.md), added 2026-06-03 after B1/B2/A2 surfaced the pattern.
5. **Update [`docs/roadmap.md`](../roadmap.md) with the slice — listed under the phase's "Complete" section with a one-line outcome and a link to the outcome doc.** A slice is **not "shipped" until the roadmap reflects it AND step 4 cleared**; this is closure work, not optional reporting. Skipping the roadmap update caused PHASE-1C3-PREP drift (six slices shipped without roadmap updates; see F-OPS-3 — *deferred work shipped earlier creates plan-vs-state drift*). The detector at `scripts/verification/check-roadmap-sync.ts` enforces presence; it does not check the `status` field's truth — that's on you and the F-OPS-6 rule above.
6. **Update [`docs/STATUS.md`](../STATUS.md) — the live "resume here" file at repo root.** Rewrite the relevant section (Code-complete / Shipped / Blocked / Next operator action) to reflect the slice. Roadmap stays as the phase narrative; STATUS is the single file a returning operator reads first to know where to pick up. A slice is **not "shipped" until STATUS reflects it** (same contract as roadmap, established 2026-06-03).
7. If new risks surfaced, add them to the risk register (see workflow below).
