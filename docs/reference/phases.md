...
| `PHASE-2A` | Phase 2a | In progress | Calibration robustness **and world-landmark activation**. Two parallel tracks: **Track A** grows the calibration ground-truth dataset from n=1 toward the ADR-0004 re-open threshold and verifies cross-clip determinism within ±1% per ADR-0005. **Track B** captures and plumbs BlazePose GHUM 3D world landmarks end-to-end as an alternative coordinate space available to metric definitions. Track A and Track B are independent; neither subsumes the other. | [`reference/calibration-audit-rollup.md`](../reference/calibration-audit-rollup.md), [`adr/0004-calibration-defer-b2-decision.md`](../adr/0004-calibration-defer-b2-decision.md), [`investigations/world-landmark-feasibility-research.md`](../investigations/world-landmark-feasibility-research.md) |
...
`PHASE-2A-SLICE-B2` (world-landmark plumbing through `AnalyzeResponse`, this registry) is **not** the "Slice B2 calibration decision" deferred by [ADR-0004](../adr/0004-calibration-defer-b2-decision.md). The latter is a calibration architecture decision gated on Track A reaching n≥3 across ≥2 filming contexts, and will be resolved by a follow-on ADR (not by a Phase 2a slice). The two share a letter for historical reasons — ADR-0004 predates the Phase 2a slice registry — and they are different work on different tracks with different gates. Conflating them was the failure mode that opened [F-OPS-6](../risk-register/F-OPS-6-verification-deferral-across-slice-boundaries.md)'s adjacent context during Phase 2a kickoff.
