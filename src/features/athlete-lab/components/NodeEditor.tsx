...
const TABS: { key: TabKey; label: string; icon: string; subtitle: string }[] = [
  ...
  // Slice E E.5 recovery (2026-04-26): Mechanics tab hidden — pro_mechanics column
  // dropped in migration 20260426025918, MechanicsEditor crashes on mount. Component
  // deletion deferred to Phase 1c.3. See F-SLICE-E-4 in migration-risk-register.md.
...
function EliteVideosEditor({ videos, onChange }: { videos: EliteVideo[]; onChange: (v: EliteVideo[]) => void }) {
  ...
  const idealCriteria = [
    ...
    "Best tier: Yard lines visible in frame — enables dynamic calibration for maximum distance measurement precision",
    "Good tier: Cones at known distances (e.g., 5 yards apart) — the system uses the node's default calibration but consistent cone spacing supports accurate drill setup",
    "Minimum tier: No visible markers still works — the system falls back to the node's Reference tab calibration settings; distance metrics remain usable with slightly reduced precision",
    "Crisp plant-and-break footwork",
    "Natural game speed (no slow-motion)",
    "Neutral lighting and field conditions",
    "5\u201310 second clip duration",
    "HD quality (1080p or higher)",
  ];
  ...
  const avoidItems = [
    ...
    "Low-light or night-game footage",
    "Instructional overlays, arrows, or graphics",
  ];
