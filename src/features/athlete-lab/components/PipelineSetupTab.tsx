...
const PHASES: Phase[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { item_id: "dynamic_calibration_implemented", title: "Dynamic field line calibration implemented", description: "Cloud Run service uses OpenCV to detect yard line stripes in the first 10 frames of each uploaded clip. Calculates pixels_per_yard dynamically per video based on detected line spacing. Falls back to node's static pixels_per_yard from reference_calibrations if detection fails. Eliminates distance metric error caused by variable filming distances across athletes. Implementation: cv2.Canny edge detection → HoughLinesP to detect horizontal stripes → measure pixel distance between consecutive lines → divide by known yard line spacing (5 yards) → return pixels_per_yard to Edge Function in response payload." },
